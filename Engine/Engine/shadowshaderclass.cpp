...
bool ShadowShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	...
	XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor,XMFLOAT3 cameraPos,XMFLOAT4 specularColor, float specularPower)
{
	bool result;
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, lightViewMatrix, lightProjectionMatrix, texture,
		depthMapTexture, lightPosition, ambientColor, diffuseColor, cameraPos, specularColor, specularPower);
	if (!result)
	{
		return false;
	}
	...
	XMFLOAT4 ambientColor, XMFLOAT4 diffuseColor, XMFLOAT3 cameraPos,XMFLOAT4 specularColor,float specularPower)
{
	...
	dataPtr4->cameraPosition = cameraPos;
	...
	dataPtr2->bufferPadding = XMFLOAT3(0.0f,0.0f,0.0f);
